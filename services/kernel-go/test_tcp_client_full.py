...
def main():
    ...
    print("Testing Complete Go Kernel Docker Integration")
    ...
    # Test start process
    print("\n2. Start Process:")
    response = send_message(host, port, 1, "test456 hello-node node app.js")
    
    # Extract process ID if successful
    process_id = None
    if response and "successfully:" in response:
        process_id = response.split("successfully: ")[1].strip()
    ...
    # Test stop the correct process
    if process_id:
        print(f"\n4. Stop Process ({process_id}):")
        send_message(host, port, 2, process_id)
        
        print("\n5. List Processes (after stop):")
        send_message(host, port, 4, "")
